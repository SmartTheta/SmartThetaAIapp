    ...
    return (
        <div className="space-y-6 text-left">
            ...
            <div className="space-y-3">
                {currQ.options.map((o) => (
                    <button
                        ...
                        className={`w-full p-6 rounded-2xl border-2 text-left text-[14px] font-bold transition-all ${riskAnswers[currQ.id] === o.score ? 'border-blue-600 bg-blue-50 text-blue-900 shadow-md' : 'border-slate-50 hover:border-slate-200 hover:bg-slate-50 text-slate-600'}`}
                    >
            ...
            <div className="flex justify-between items-center pt-2">
                <button
                    onClick={onBack}
                    className="text-xs font-bold text-slate-400 flex items-center gap-1 uppercase tracking-widest underline decoration-dotted hover:text-slate-600 transition-colors"
                >
                ...
                <div className="flex flex-col items-end">
                    ...
                    <span className="text-[10px] font-bold text-slate-400 uppercase">{progress}% Complete</span>
                </div>
            </div>
        </div>
    );
